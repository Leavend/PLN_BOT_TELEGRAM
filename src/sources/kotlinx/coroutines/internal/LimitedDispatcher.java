...
public final class LimitedDispatcher extends CoroutineDispatcher implements Delay {
    ...
    public void mo8412scheduleResumeAfterDelay(long timeMillis, CancellableContinuation<? super Unit> continuation) {
        this.$$delegate_0.mo8412scheduleResumeAfterDelay(timeMillis, continuation);
    }
        ...
        public void run() {
            ...
            while (true) {
                ...
                if (i >= 16 && LimitedDispatcher.this.dispatcher.isDispatchNeeded(LimitedDispatcher.this)) {
                    LimitedDispatcher.this.dispatcher.mo8411dispatch(LimitedDispatcher.this, this);
                    return;
                }
            }
        }
    ...
    public void mo8411dispatch(CoroutineContext context, Runnable block) {
        Runnable runnableObtainTaskOrDeallocateWorker;
        this.queue.addLast(block);
        if (runningWorkers$FU.get(this) >= this.parallelism || !tryAllocateWorker() || (runnableObtainTaskOrDeallocateWorker = obtainTaskOrDeallocateWorker()) == null) {
            return;
        }
        this.dispatcher.mo8411dispatch(this, new Worker(runnableObtainTaskOrDeallocateWorker));
    }
