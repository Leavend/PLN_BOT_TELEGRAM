    ...
    public static final <S extends Segment<S>> Object findSegmentInternal(S s, long j, Function2<? super Long, ? super S, ? extends S> function2) {
        while (true) {
            if (s.id >= j && !s.isRemoved()) {
                return SegmentOrClosed.m8414constructorimpl(s);
            }
            Object nextOrClosed = s.getNextOrClosed();
            if (nextOrClosed == CLOSED) {
                return SegmentOrClosed.m8414constructorimpl(CLOSED);
            }
        ...
        do {
            ...
            if (!SegmentOrClosed.m8419isClosedimpl(objFindSegmentInternal)) {
                ...
                while (true) {
                    Segment segment2 = (Segment) atomicReferenceFieldUpdater.get(obj);
                    z = true;
                    if (segment2.id >= segmentM8417getSegmentimpl.id) {
                        break;
                    }
                    ...
                    if (AbstractResolvableFuture$SafeAtomicHelper$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceFieldUpdater, obj, segment2, segmentM8417getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                        }
        ...
        do {
            ...
            if (!SegmentOrClosed.m8419isClosedimpl(objFindSegmentInternal)) {
                ...
                while (true) {
                    ...
                    if (Striped$SmallLazyStriped$$ExternalSyntheticBackportWithForwarding0.m(atomicReferenceArray, i, segment2, segmentM8417getSegmentimpl)) {
                        if (segment2.decPointers$kotlinx_coroutines_core()) {
                            segment2.remove();
                        }
                    } else if (segmentM8417getSegmentimpl.decPointers$kotlinx_coroutines_core()) {
