...
final class ResumeAwaitOnCompletion<T> extends JobNode {
    ...
    public void invoke2(Throwable cause) {
        ...
        if (state$kotlinx_coroutines_core instanceof CompletedExceptionally) {
            ...
            cancellableContinuationImpl.resumeWith(Result.m6852constructorimpl(ResultKt.createFailure(((CompletedExceptionally) state$kotlinx_coroutines_core).cause)));
        } else {
            CancellableContinuationImpl<T> cancellableContinuationImpl2 = this.continuation;
            Result.Companion companion2 = Result.INSTANCE;
            cancellableContinuationImpl2.resumeWith(Result.m6852constructorimpl(JobSupportKt.unboxState(state$kotlinx_coroutines_core)));
        }
    }
}
