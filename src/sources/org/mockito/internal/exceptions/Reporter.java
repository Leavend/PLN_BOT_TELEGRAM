...
public class Reporter {
    ...
    public static MockitoException stubPassedToVerify() {
        return new CannotVerifyStubOnlyMock(StringUtil.join("Argument passed to verify() is a stubOnly() mock, not a full blown mock!", "If you intend to verify invocations on a mock, don't use stubOnly() in its MockSettings."));
    }
    ...
    public static MockitoAssertionError wantedButNotInvoked(DescribedInvocation describedInvocation, List<? extends DescribedInvocation> list) {
        ...
        if (list.isEmpty()) {
            ...
            for (DescribedInvocation describedInvocation2 : list) {
                sb.append(describedInvocation2.toString()).append("\n").append(describedInvocation2.getLocation()).append("\n\n");
            }
            string = sb.toString();
        }
        return new WantedButNotInvoked(createWantedButNotInvokedMessage(describedInvocation) + string);
    }
    ...
    public static MockitoAssertionError tooLittleActualInvocationsInOrder(Discrepancy discrepancy, DescribedInvocation describedInvocation, List<Location> list) {
        return new VerificationInOrderFailure(StringUtil.join("Verification in order failure:" + createTooLittleInvocationsMessage(discrepancy, describedInvocation, list)));
    }

    public static MockitoAssertionError noMoreInteractionsWanted(Invocation invocation, List<VerificationAwareInvocation> list) {
        return new NoInteractionsWanted(StringUtil.join("No interactions wanted here:", new LocationImpl(), "But found this interaction on mock '" + safelyGetMockName(invocation.getMock()) + "':", invocation.getLocation(), new ScenarioPrinter().print(list)));
    }

    public static MockitoAssertionError noMoreInteractionsWantedInOrder(Invocation invocation) {
        return new VerificationInOrderFailure(StringUtil.join("No interactions wanted here:", new LocationImpl(), "But found this interaction on mock '" + safelyGetMockName(invocation.getMock()) + "':", invocation.getLocation()));
    }
    ...
    public static MockitoException wrongTypeReturnedByDefaultAnswer(Object obj, String str, String str2, String str3) {
        return new WrongTypeOfReturnValue(StringUtil.join("Default answer returned a result with the wrong type:", str2 + " cannot be returned by " + str3 + "()", str3 + "() should return " + str, "", "The default answer of " + safelyGetMockName(obj) + " that was configured on the mock is probably incorrectly implemented.", ""));
    }

    public static MockitoAssertionError wantedAtMostX(int i, int i2) {
        return new MockitoAssertionError(StringUtil.join("Wanted at most " + Pluralizer.pluralize(i) + " but was " + i2));
    }
    ...
    public static MockitoException cannotInjectDependency(Field field, Object obj, Exception exc) {
        return new MockitoException(StringUtil.join("Mockito couldn't inject mock dependency '" + safelyGetMockName(obj) + "' on field ", "'" + field + "'", "whose type '" + field.getDeclaringClass().getCanonicalName() + "' was annotated by @InjectMocks in your test.", "Also I failed because: " + exceptionCauseMessageIfAvailable(exc), ""), exc);
    }
    ...
    public static MockitoException invalidArgumentPositionRangeAtInvocationTime(InvocationOnMock invocationOnMock, boolean z, int i) {
        ...
        objArr[1] = " -> " + safelyGetMockName(invocationOnMock.getMock()) + "." + invocationOnMock.getMethod().getName() + "()";
        objArr[2] = "";
        objArr[3] = (z ? "Last parameter wanted" : "Wanted parameter at position " + i) + " but " + ((Object) possibleArgumentTypesOf(invocationOnMock));
    ...
    private static StringBuilder possibleArgumentTypesOf(InvocationOnMock invocationOnMock) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            if (invocationOnMock.getMethod().isVarArgs() && i == length - 1) {
                sb.append("+] ").append(parameterTypes[i].getComponentType().getSimpleName()).append("  <- Vararg\n");
            } else {
                sb.append("] ").append(parameterTypes[i].getSimpleName()).append("\n");
            }
        }
        return sb;
    }
    ...
    public static MockitoException wrongTypeOfArgumentToReturn(InvocationOnMock invocationOnMock, String str, Class<?> cls, int i) {
        return new WrongTypeOfReturnValue(StringUtil.join("The argument of type '" + cls.getSimpleName() + "' cannot be returned because the following ", "method should return the type '" + str + "'", " -> " + safelyGetMockName(invocationOnMock.getMock()) + "." + invocationOnMock.getMethod().getName() + "()", "", "The reason for this error can be :", "1. The wanted argument position is incorrect.", "2. The answer is used on the wrong interaction.", "", "Position of the wanted argument is " + i + " and " + ((Object) possibleArgumentTypesOf(invocationOnMock)), "***", "However if you're still unsure why you're getting above error read on.", "Due to the nature of the syntax above problem might occur because:", "1. This exception *might* occur in wrongly written multi-threaded tests.", "   Please refer to Mockito FAQ on limitations of concurrency testing.", "2. A spy is stubbed using when(spy.foo()).then() syntax. It is safer to stub spies - ", "   - with doReturn|Throw() family of methods. More in javadocs for Mockito.spy() method.", ""));
    }
    ...
    public static MockitoException delegatedMethodHasWrongReturnType(Method method, Method method2, Object obj, Object obj2) {
        return new MockitoException(StringUtil.join("Methods called on delegated instance must have compatible return types with the mock.", "When calling: " + method + " on mock: " + safelyGetMockName(obj), "return type should be: " + method.getReturnType().getSimpleName() + ", but was: " + method2.getReturnType().getSimpleName(), "Check that the instance passed to delegatesTo() is of the correct type or contains compatible methods", "(delegate instance had type: " + obj2.getClass().getSimpleName() + ")"));
    }

    public static MockitoException delegatedMethodDoesNotExistOnDelegate(Method method, Object obj, Object obj2) {
        return new MockitoException(StringUtil.join("Methods called on mock must exist in delegated instance.", "When calling: " + method + " on mock: " + safelyGetMockName(obj), "no such method was found.", "Check that the instance passed to delegatesTo() is of the correct type or contains compatible methods", "(delegate instance had type: " + obj2.getClass().getSimpleName() + ")"));
    }
    ...
    private static MockName safelyGetMockName(Object obj) {
        return MockUtil.getMockName(obj);
    }
    ...
    public static UnnecessaryStubbingException formatUnncessaryStubbingException(Class<?> cls, Collection<Invocation> collection) {
        ...
        int i = 1;
        while (it.hasNext()) {
            sb.append("\n  ").append(i).append(". ").append(it.next().getLocation());
            i++;
        }
    ...
    public static void potentialStubbingProblem(Invocation invocation, Collection<Invocation> collection) {
        ...
        for (Invocation invocation2 : collection) {
            sb.append("    ").append(i).append(". ").append(invocation2);
            sb.append("\n      ").append(invocation2.getLocation()).append("\n");
            i++;
        }
