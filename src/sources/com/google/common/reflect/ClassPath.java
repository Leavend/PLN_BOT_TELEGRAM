...
public final class ClassPath {
        ...
        if (value != null) {
            for (String str : CLASS_PATH_ATTRIBUTE_SEPARATOR.split(value)) {
                try {
                    ...
                    if (classPathEntry.getProtocol().equals(StorageChooser.FILE_PICKER)) {
    ...
    static ImmutableMap<File, ClassLoader> getClassPathEntries(ClassLoader classLoader) {
        ...
        while (it.hasNext()) {
            URL next = it.next();
            if (next.getProtocol().equals(StorageChooser.FILE_PICKER)) {
        ...
        for (String str : Splitter.on(StandardSystemProperty.PATH_SEPARATOR.value()).split(StandardSystemProperty.JAVA_CLASS_PATH.value())) {
            try {
                try {
                    ...
                    builder.add((ImmutableList.Builder) new URL(StorageChooser.FILE_PICKER, (String) null, new File(str).getAbsolutePath()));
                }
    ...
    static File toFile(URL url) {
        Preconditions.checkArgument(url.getProtocol().equals(StorageChooser.FILE_PICKER));
        try {
