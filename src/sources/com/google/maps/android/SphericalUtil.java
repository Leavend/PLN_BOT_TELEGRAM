...
public class SphericalUtil {
    ...
    public static double computeLength(List<LatLng> list) {
        int size = list.size();
        double dDistanceRadians = FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE;
        if (size < 2) {
        ...
        while (true) {
            ...
            dDistanceRadians += distanceRadians(d2, d, radians, radians2);
        }
    }
    ...
    static double computeSignedArea(List<LatLng> list, double d) {
        int size = list.size();
        double dPolarTriangleArea = FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE;
        if (size < 3) {
            return FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE;
        }
