...
public class PolyUtil {
    ...
    public static boolean containsLocation(double d, double d2, List<LatLng> list, boolean z) {
        ...
        for (LatLng latLng2 : list) {
            ...
            if (radians == d3 && dWrap == FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE) {
                return true;
            }
            ...
            while (it.hasNext()) {
                ...
                if (Math.max(radians3, radians7) < d5 || Math.min(radians3, radians7) > d7) {
                    ...
                    for (int i2 = 0; i2 < 3; i2++) {
                        ...
                        double dClamp = FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE;
                        if (d10 > FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE) {
                            dClamp = MathUtil.clamp(((d8 * dWrap) + ((dMercator2 - dMercator) * d9)) / d10, FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE, 1.0d);
                        }
    ...
    private static boolean isOnSegmentGC(double d, double d2, double d3, double d4, double d5, double d6, double d7) {
        ...
        return MathUtil.sinSumFromHav((dHavDistance - dHavFromSin) / d9, (dHavDistance2 - dHavFromSin) / d9) > FirebaseRemoteConfig.DEFAULT_VALUE_FOR_DOUBLE;
    }
