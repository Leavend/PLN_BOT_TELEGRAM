...
class OkHttpClientStream extends AbstractClientStream {
    ...
    class TransportState extends Http2ClientStreamTransportState {
        ...
        public void start(int i) {
            Preconditions.checkState(OkHttpClientStream.this.f167id == -1, "the stream has been started with id %s", i);
            OkHttpClientStream.this.f167id = i;
            OkHttpClientStream.this.state.onStreamAllocated();
            if (this.canStart) {
                this.frameWriter.synStream(OkHttpClientStream.this.useGet, false, OkHttpClientStream.this.f167id, 0, this.requestHeaders);
                OkHttpClientStream.this.statsTraceCtx.clientOutboundHeaders();
                this.requestHeaders = null;
                if (this.pendingData.size() > 0) {
                    this.outboundFlow.data(this.pendingDataHasEndOfStream, OkHttpClientStream.this.f167id, this.pendingData, this.flushPendingData);
                }
                this.canStart = false;
            }
        }
