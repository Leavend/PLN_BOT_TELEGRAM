        ...
        while (statusSendTap != Tapper.Status.SUCCESS && i < 3) {
            try {
                ...
                if (Log.isLoggable(TAG, 3)) {
                    Log.d(TAG, "perform: " + String.format(Locale.ROOT, "%s - At Coordinates: %d, %d and precision: %d, %d", getDescription(), Integer.valueOf((int) fArrCalculateCoordinates[0]), Integer.valueOf((int) fArrCalculateCoordinates[1]), Integer.valueOf((int) fArrDescribePrecision[0]), Integer.valueOf((int) fArrDescribePrecision[1])));
                }
