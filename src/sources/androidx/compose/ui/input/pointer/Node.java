    ...
    */
    ...
    private final boolean hasPositionChanged(PointerEvent oldEvent, PointerEvent newEvent) {
        ...
        for (int i = 0; i < size; i++) {
            if (!Offset.m3170equalsimpl0(oldEvent.getChanges().get(i).getPosition(), newEvent.getChanges().get(i).getPosition())) {
                return true;
            }
        }
        return false;
    ...
    public void cleanUpHits(InternalPointerEvent internalPointerEvent) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if ((pointerInputChange.getPressed() || (internalPointerEvent.m4487issuesEnterExitEvent0FcD4WY(pointerInputChange.getId()) && this.isIn)) ? false : true) {
                this.pointerIds.m4654remove0FcD4WY(pointerInputChange.getId());
            }
        }
        this.isIn = false;
        this.hasExited = PointerEventType.m4507equalsimpl0(pointerEvent.getType(), PointerEventType.INSTANCE.m4512getExit7fucELk());
    }
    ...
    public boolean dispatchMainEventPass(LongSparseArray<PointerInputChange> changes, LayoutCoordinates parentCoordinates, InternalPointerEvent internalPointerEvent, boolean isInBounds) {
        ...
        while (delegatingNodePop != 0) {
            if (delegatingNodePop instanceof PointerInputModifierNode) {
                ((PointerInputModifierNode) delegatingNodePop).mo181onPointerEventH0pRuoY(pointerEvent, PointerEventPass.Initial, jMo4691getSizeYbymL2g);
            } else {
                if (((delegatingNodePop.getKindSet() & iM4891constructorimpl) != 0) && (delegatingNodePop instanceof DelegatingNode)) {
                    Modifier.Node delegate = delegatingNodePop.getDelegate();
                    int i = 0;
        ...
        if (this.modifierNode.getIsAttached()) {
            ...
            int iM4891constructorimpl2 = NodeKind.m4891constructorimpl(16);
            MutableVector mutableVector2 = null;
            while (delegatingNodePop2 != 0) {
                if (delegatingNodePop2 instanceof PointerInputModifierNode) {
                    ((PointerInputModifierNode) delegatingNodePop2).mo181onPointerEventH0pRuoY(pointerEvent, PointerEventPass.Main, jMo4691getSizeYbymL2g);
                } else {
                    if (((delegatingNodePop2.getKindSet() & iM4891constructorimpl2) != 0) && (delegatingNodePop2 instanceof DelegatingNode)) {
    ...
    public boolean dispatchFinalEventPass(InternalPointerEvent internalPointerEvent) {
        ...
        if (!this.relevantChanges.isEmpty() && this.modifierNode.getIsAttached()) {
            ...
            while (delegatingNodePop != 0) {
                if (delegatingNodePop instanceof PointerInputModifierNode) {
                    ((PointerInputModifierNode) delegatingNodePop).mo181onPointerEventH0pRuoY(pointerEvent, PointerEventPass.Final, jMo4691getSizeYbymL2g);
                } else {
                    if (((delegatingNodePop.getKindSet() & iM4891constructorimpl) != 0) && (delegatingNodePop instanceof DelegatingNode)) {
                        Modifier.Node delegate = delegatingNodePop.getDelegate();
                        int i2 = 0;
