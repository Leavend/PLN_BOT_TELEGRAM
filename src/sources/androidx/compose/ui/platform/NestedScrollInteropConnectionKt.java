...
public final class NestedScrollInteropConnectionKt {
    ...
    public static final long m5032toOffsetUv8p0NA(int[] iArr, long j) {
        float fCoerceAtLeast;
        float fCoerceAtLeast2;
        if (Offset.m3173getXimpl(j) >= 0.0f) {
            fCoerceAtLeast = RangesKt.coerceAtMost(reverseAxis(iArr[0]), Offset.m3173getXimpl(j));
        } else {
            fCoerceAtLeast = RangesKt.coerceAtLeast(reverseAxis(iArr[0]), Offset.m3173getXimpl(j));
        }
        if (Offset.m3174getYimpl(j) >= 0.0f) {
            fCoerceAtLeast2 = RangesKt.coerceAtMost(reverseAxis(iArr[1]), Offset.m3174getYimpl(j));
        } else {
            fCoerceAtLeast2 = RangesKt.coerceAtLeast(reverseAxis(iArr[1]), Offset.m3174getYimpl(j));
        }
    ...
    public static final int m5033toViewTypeGyEprt8(int i) {
        return !NestedScrollSource.m4447equalsimpl0(i, NestedScrollSource.INSTANCE.m4452getDragWNlRxjI()) ? 1 : 0;
    }

    /* JADX INFO: Access modifiers changed from: private */
    /* renamed from: getScrollAxes-k-4lQ0M, reason: not valid java name */
    public static final int m5031getScrollAxesk4lQ0M(long j) {
        int i = Math.abs(Offset.m3173getXimpl(j)) >= 0.5f ? 1 : 0;
        return Math.abs(Offset.m3174getYimpl(j)) >= 0.5f ? i | 2 : i;
    }
