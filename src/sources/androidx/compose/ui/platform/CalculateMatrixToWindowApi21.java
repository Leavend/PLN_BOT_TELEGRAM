...
final class CalculateMatrixToWindowApi21 implements CalculateMatrixToWindow {
    ...
    public void mo4986calculateMatrixToWindowEL8BTi8(View view, float[] matrix) {
        Matrix.m3654resetimpl(matrix);
        m4989transformMatrixToWindowEL8BTi8(view, matrix);
    }

    /* renamed from: transformMatrixToWindow-EL8BTi8, reason: not valid java name */
    private final void m4989transformMatrixToWindowEL8BTi8(View view, float[] matrix) {
        Object parent = view.getParent();
        if (parent instanceof View) {
            m4989transformMatrixToWindowEL8BTi8((View) parent, matrix);
            m4988preTranslate3XD1CNM(matrix, -view.getScrollX(), -view.getScrollY());
            m4988preTranslate3XD1CNM(matrix, view.getLeft(), view.getTop());
        } else {
            view.getLocationInWindow(this.tmpLocation);
            m4988preTranslate3XD1CNM(matrix, -view.getScrollX(), -view.getScrollY());
            m4988preTranslate3XD1CNM(matrix, r0[0], r0[1]);
        }
        android.graphics.Matrix matrix2 = view.getMatrix();
        if (matrix2.isIdentity()) {
            return;
        }
        m4987preConcattUYjHk(matrix, matrix2);
    }

    /* renamed from: preConcat-tU-YjHk, reason: not valid java name */
    private final void m4987preConcattUYjHk(float[] fArr, android.graphics.Matrix matrix) {
        AndroidMatrixConversions_androidKt.m3285setFromtUYjHk(this.tmpMatrix, matrix);
        AndroidComposeView_androidKt.m4984preTransformJiSxe2E(fArr, this.tmpMatrix);
    }

    /* renamed from: preTranslate-3XD1CNM, reason: not valid java name */
    private final void m4988preTranslate3XD1CNM(float[] fArr, float f, float f2) {
        AndroidComposeView_androidKt.m4985preTranslatecG2Xzmc(fArr, f, f2, this.tmpMatrix);
    }
}
