...
public final /* data */ class BiasAbsoluteAlignment implements Alignment {
    ...
    public long mo3050alignKFBX0sM(long size, long space, LayoutDirection layoutDirection) {
        long jIntSize = IntSizeKt.IntSize(IntSize.m5905getWidthimpl(space) - IntSize.m5905getWidthimpl(size), IntSize.m5904getHeightimpl(space) - IntSize.m5904getHeightimpl(size));
        float f = 1;
        return IntOffsetKt.IntOffset(MathKt.roundToInt((IntSize.m5905getWidthimpl(jIntSize) / 2.0f) * (this.horizontalBias + f)), MathKt.roundToInt((IntSize.m5904getHeightimpl(jIntSize) / 2.0f) * (f + this.verticalBias)));
    }
