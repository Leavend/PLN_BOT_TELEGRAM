...
public final class SweepGradient extends ShaderBrush {
    ...
    public Shader mo3383createShaderuvyYCjk(long size) {
        ...
        if (OffsetKt.m3194isUnspecifiedk4lQ0M(this.center)) {
            jOffset = SizeKt.m3252getCenteruvyYCjk(size);
        } else {
            jOffset = OffsetKt.Offset((Offset.m3173getXimpl(this.center) > Float.POSITIVE_INFINITY ? 1 : (Offset.m3173getXimpl(this.center) == Float.POSITIVE_INFINITY ? 0 : -1)) == 0 ? Size.m3242getWidthimpl(size) : Offset.m3173getXimpl(this.center), Offset.m3174getYimpl(this.center) == Float.POSITIVE_INFINITY ? Size.m3239getHeightimpl(size) : Offset.m3174getYimpl(this.center));
        }
        return ShaderKt.m3732SweepGradientShader9KIMszo(jOffset, this.colors, this.stops);
    }
    ...
    public boolean equals(Object other) {
        ...
        return Offset.m3170equalsimpl0(this.center, sweepGradient.center) && Intrinsics.areEqual(this.colors, sweepGradient.colors) && Intrinsics.areEqual(this.stops, sweepGradient.stops);
    }

    public int hashCode() {
        int iM3175hashCodeimpl = ((Offset.m3175hashCodeimpl(this.center) * 31) + this.colors.hashCode()) * 31;
        List<Float> list = this.stops;
        return iM3175hashCodeimpl + (list != null ? list.hashCode() : 0);
    }

    public String toString() {
        return "SweepGradient(" + (OffsetKt.m3192isSpecifiedk4lQ0M(this.center) ? "center=" + ((Object) Offset.m3181toStringimpl(this.center)) + ", " : "") + "colors=" + this.colors + ", stops=" + this.stops + ')';
    }
}
