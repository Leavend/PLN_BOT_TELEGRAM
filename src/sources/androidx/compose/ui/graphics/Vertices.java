...
public final class Vertices {
    ...
    private final int[] encodeColorList(List<Color> colors) {
        ...
        for (int i = 0; i < size; i++) {
            iArr[i] = ColorKt.m3468toArgb8_81llA(colors.get(i).m3424unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> points) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (i % 2 == 0) {
                fM3174getYimpl = Offset.m3173getXimpl(packedValue);
            } else {
                fM3174getYimpl = Offset.m3174getYimpl(packedValue);
            }
            fArr[i] = fM3174getYimpl;
        }
        return fArr;
    }
}
