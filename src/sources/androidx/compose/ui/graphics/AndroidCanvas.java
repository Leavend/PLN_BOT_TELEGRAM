...
public final class AndroidCanvas implements Canvas {
    ...
    public void mo3269concat58bKbWc(float[] matrix) {
        if (MatrixKt.m3669isIdentity58bKbWc(matrix)) {
            return;
        }
        android.graphics.Matrix matrix2 = new android.graphics.Matrix();
        AndroidMatrixConversions_androidKt.m3284setFromEL8BTi8(matrix2, matrix);
        this.internalCanvas.concat(matrix2);
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: clipRect-N_I0leg, reason: not valid java name */
    public void mo3268clipRectN_I0leg(float left, float top, float right, float bottom, int clipOp) {
        this.internalCanvas.clipRect(left, top, right, bottom, m3277toRegionOp7u2Bmg(clipOp));
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: clipPath-mtrdD-E, reason: not valid java name */
    public void mo3267clipPathmtrdDE(Path path, int clipOp) {
        android.graphics.Canvas canvas = this.internalCanvas;
        if (path instanceof AndroidPath) {
            canvas.clipPath(((AndroidPath) path).getInternalPath(), m3277toRegionOp7u2Bmg(clipOp));
            return;
        }
    ...
    public final Region.Op m3277toRegionOp7u2Bmg(int i) {
        return ClipOp.m3398equalsimpl0(i, ClipOp.INSTANCE.m3402getDifferencertfAjoo()) ? Region.Op.DIFFERENCE : Region.Op.INTERSECT;
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawLine-Wko1d7g, reason: not valid java name */
    public void mo3273drawLineWko1d7g(long p1, long p2, Paint paint) {
        this.internalCanvas.drawLine(Offset.m3173getXimpl(p1), Offset.m3174getYimpl(p1), Offset.m3173getXimpl(p2), Offset.m3174getYimpl(p2), paint.getInternalPaint());
    }
    ...
    public void mo3270drawCircle9KIMszo(long center, float radius, Paint paint) {
        this.internalCanvas.drawCircle(Offset.m3173getXimpl(center), Offset.m3174getYimpl(center), radius, paint.getInternalPaint());
    }
    ...
    public void mo3271drawImaged4ec7I(ImageBitmap image, long topLeftOffset, Paint paint) {
        this.internalCanvas.drawBitmap(AndroidImageBitmap_androidKt.asAndroidBitmap(image), Offset.m3173getXimpl(topLeftOffset), Offset.m3174getYimpl(topLeftOffset), paint.getInternalPaint());
    }

    @Override // androidx.compose.ui.graphics.Canvas
    /* renamed from: drawImageRect-HPBpro0, reason: not valid java name */
    public void mo3272drawImageRectHPBpro0(ImageBitmap image, long srcOffset, long srcSize, long dstOffset, long dstSize, Paint paint) {
        ...
        rect.left = IntOffset.m5863getXimpl(srcOffset);
        rect.top = IntOffset.m5864getYimpl(srcOffset);
        rect.right = IntOffset.m5863getXimpl(srcOffset) + IntSize.m5905getWidthimpl(srcSize);
        rect.bottom = IntOffset.m5864getYimpl(srcOffset) + IntSize.m5904getHeightimpl(srcSize);
        Unit unit = Unit.INSTANCE;
        Rect rect2 = this.dstRect;
        Intrinsics.checkNotNull(rect2);
        rect2.left = IntOffset.m5863getXimpl(dstOffset);
        rect2.top = IntOffset.m5864getYimpl(dstOffset);
        rect2.right = IntOffset.m5863getXimpl(dstOffset) + IntSize.m5905getWidthimpl(dstSize);
        rect2.bottom = IntOffset.m5864getYimpl(dstOffset) + IntSize.m5904getHeightimpl(dstSize);
    ...
    public void mo3274drawPointsO7TthRY(int pointMode, List<Offset> points, Paint paint) {
        if (PointMode.m3714equalsimpl0(pointMode, PointMode.INSTANCE.m3718getLinesr_lszbg())) {
            drawLines(points, paint, 2);
        } else if (PointMode.m3714equalsimpl0(pointMode, PointMode.INSTANCE.m3720getPolygonr_lszbg())) {
            drawLines(points, paint, 1);
        } else if (PointMode.m3714equalsimpl0(pointMode, PointMode.INSTANCE.m3719getPointsr_lszbg())) {
    ...
    private final void drawLines(List<Offset> points, Paint paint, int stepBy) {
        if (points.size() >= 2) {
            ...
            while (i < points.size() - 1) {
                ...
                this.internalCanvas.drawLine(Offset.m3173getXimpl(packedValue), Offset.m3174getYimpl(packedValue), Offset.m3173getXimpl(packedValue2), Offset.m3174getYimpl(packedValue2), internalPaint);
                i += stepBy;
            }
        }
    }
    ...
    public void mo3275drawRawPointsO7TthRY(int pointMode, float[] points, Paint paint) {
    ...
    public void mo3276drawVerticesTPEHhCM(Vertices vertices, int blendMode, Paint paint) {
        this.internalCanvas.drawVertices(AndroidVertexMode_androidKt.m3320toAndroidVertexModeJOOmi9M(vertices.getVertexMode()), vertices.getPositions().length, vertices.getPositions(), 0, vertices.getTextureCoordinates(), 0, vertices.getColors(), 0, vertices.getIndices(), 0, vertices.getIndices().length, paint.getInternalPaint());
    }
    ...
    private final void drawPoints(List<Offset> points, Paint paint) {
        ...
        for (int i = 0; i < size; i++) {
            long packedValue = points.get(i).getPackedValue();
            this.internalCanvas.drawPoint(Offset.m3173getXimpl(packedValue), Offset.m3174getYimpl(packedValue), paint.getInternalPaint());
        }
    }
}
