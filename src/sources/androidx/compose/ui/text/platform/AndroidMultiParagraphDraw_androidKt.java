...
public final class AndroidMultiParagraphDraw_androidKt {
    ...
    public static final void m5479drawMultiParagraph7AXcY_I(MultiParagraph multiParagraph, Canvas canvas, Brush brush, float f, Shadow shadow, TextDecoration textDecoration, DrawStyle drawStyle, int i) {
        canvas.save();
        if (multiParagraph.getParagraphInfoList$ui_text_release().size() <= 1 || (brush instanceof SolidColor)) {
            m5481drawParagraphs7AXcY_I(multiParagraph, canvas, brush, f, shadow, textDecoration, drawStyle, i);
            ...
            Shader shaderMo3383createShaderuvyYCjk = ((ShaderBrush) brush).mo3383createShaderuvyYCjk(SizeKt.Size(fMax, height));
            Matrix matrix = new Matrix();
            shaderMo3383createShaderuvyYCjk.getLocalMatrix(matrix);
            ...
            for (int i3 = 0; i3 < size2; i3++) {
                ...
                paragraphInfo2.getParagraph().mo5087painthn5TExg(canvas, BrushKt.ShaderBrush(shaderMo3383createShaderuvyYCjk), f, shadow, textDecoration, drawStyle, i);
                ...
                shaderMo3383createShaderuvyYCjk.setLocalMatrix(matrix);
            }
        }
        canvas.restore();
    }
    ...
    private static final void m5481drawParagraphs7AXcY_I(MultiParagraph multiParagraph, Canvas canvas, Brush brush, float f, Shadow shadow, TextDecoration textDecoration, DrawStyle drawStyle, int i) {
        List<ParagraphInfo> paragraphInfoList$ui_text_release = multiParagraph.getParagraphInfoList$ui_text_release();
        int size = paragraphInfoList$ui_text_release.size();
        for (int i2 = 0; i2 < size; i2++) {
            ParagraphInfo paragraphInfo = paragraphInfoList$ui_text_release.get(i2);
            paragraphInfo.getParagraph().mo5087painthn5TExg(canvas, brush, f, shadow, textDecoration, drawStyle, i);
