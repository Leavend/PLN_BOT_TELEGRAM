...
public final class MultiParagraph {
    ...
    private MultiParagraph(MultiParagraphIntrinsics multiParagraphIntrinsics, long j, int i, boolean z) {
        ...
        this.maxLines = i;
        int i2 = 0;
        if (!(Constraints.m5693getMinWidthimpl(j) == 0 && Constraints.m5692getMinHeightimpl(j) == 0)) {
        ...
        while (i4 < size) {
            ...
            if (Constraints.m5686getHasBoundedHeightimpl(j)) {
                iM5690getMaxHeightimpl = RangesKt.coerceAtLeast(Constraints.m5690getMaxHeightimpl(j) - ParagraphKt.ceilToInt(f), i2);
            } else {
                iM5690getMaxHeightimpl = Constraints.m5690getMaxHeightimpl(j);
            }
            Paragraph paragraphM5128Paragraph_EkL_Y = ParagraphKt.m5128Paragraph_EkL_Y(intrinsics, ConstraintsKt.Constraints$default(0, iM5691getMaxWidthimpl, 0, iM5690getMaxHeightimpl, 5, null), this.maxLines - i3, z);
            float height = f + paragraphM5128Paragraph_EkL_Y.getHeight();
            int lineCount = i3 + paragraphM5128Paragraph_EkL_Y.getLineCount();
            arrayList.add(new ParagraphInfo(paragraphM5128Paragraph_EkL_Y, paragraphIntrinsicInfo.getStartIndex(), paragraphIntrinsicInfo.getEndIndex(), i3, lineCount, f, height));
            if (paragraphM5128Paragraph_EkL_Y.getDidExceedMaxLines() || (lineCount == this.maxLines && i4 != CollectionsKt.getLastIndex(this.intrinsics.getInfoList$ui_text_release()))) {
                i3 = lineCount;
                f = height;
                z2 = true;
                break;
        ...
        this.width = Constraints.m5691getMaxWidthimpl(j);
    ...
    public static /* synthetic */ void m5109paintRPmYEkk$default(MultiParagraph multiParagraph, Canvas canvas, long j, Shadow shadow, TextDecoration textDecoration, int i, Object obj) {
        if ((i & 2) != 0) {
            j = Color.INSTANCE.m3450getUnspecified0d7_KjU();
        }
        multiParagraph.m5115paintRPmYEkk(canvas, j, (i & 4) != 0 ? null : shadow, (i & 8) != 0 ? null : textDecoration);
    }
    ...
    public final /* synthetic */ void m5115paintRPmYEkk(Canvas canvas, long color, Shadow shadow, TextDecoration decoration) {
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo5086paintRPmYEkk(canvas, color, shadow, decoration);
    ...
    public final void m5114paintLG529CI(Canvas canvas, long color, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        canvas.save();
        List<ParagraphInfo> list = this.paragraphInfoList;
        int size = list.size();
        for (int i = 0; i < size; i++) {
            ParagraphInfo paragraphInfo = list.get(i);
            paragraphInfo.getParagraph().mo5085paintLG529CI(canvas, color, shadow, decoration, drawStyle, blendMode);
            canvas.translate(0.0f, paragraphInfo.getParagraph().getHeight());
        }
        canvas.restore();
    }
    ...
    public final void m5116painthn5TExg(Canvas canvas, Brush brush, float alpha, Shadow shadow, TextDecoration decoration, DrawStyle drawStyle, int blendMode) {
        AndroidMultiParagraphDraw_androidKt.m5479drawMultiParagraph7AXcY_I(this, canvas, brush, alpha, shadow, decoration, drawStyle, blendMode);
    }
    ...
    public final Path getPathForRange(final int start, final int end) {
        ...
        MultiParagraphKt.m5117findParagraphsByRangeSbBc2M(this.paragraphInfoList, TextRangeKt.TextRange(start, end), new Function1<ParagraphInfo, Unit>() { // from class: androidx.compose.ui.text.MultiParagraph.getPathForRange.2
            ...
            public final void invoke2(ParagraphInfo paragraphInfo) {
                Path.m3684addPathUv8p0NA$default(Path, paragraphInfo.toGlobal(paragraphInfo.getParagraph().getPathForRange(paragraphInfo.toLocalIndex(start), paragraphInfo.toLocalIndex(end))), 0L, 2, null);
            }
        });
        return Path;
    }
    ...
    public final int m5112getOffsetForPositionk4lQ0M(long position) {
        int lastIndex;
        if (Offset.m3174getYimpl(position) <= 0.0f) {
            lastIndex = 0;
        } else {
            lastIndex = Offset.m3174getYimpl(position) >= this.height ? CollectionsKt.getLastIndex(this.paragraphInfoList) : MultiParagraphKt.findParagraphByY(this.paragraphInfoList, Offset.m3174getYimpl(position));
        }
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(lastIndex);
        if (paragraphInfo.getLength() == 0) {
            return paragraphInfo.getStartIndex();
        }
        return paragraphInfo.toGlobalIndex(paragraphInfo.getParagraph().mo5083getOffsetForPositionk4lQ0M(paragraphInfo.m5125toLocalMKHz9U(position)));
    }
    ...
    public final float[] m5111fillBoundingBoxes8ffj60Q(final long range, final float[] array, int arrayStart) {
        requireIndexInRange(TextRange.m5233getMinimpl(range));
        requireIndexInRangeInclusiveEnd(TextRange.m5232getMaximpl(range));
        final Ref.IntRef intRef = new Ref.IntRef();
        intRef.element = arrayStart;
        final Ref.FloatRef floatRef = new Ref.FloatRef();
        MultiParagraphKt.m5117findParagraphsByRangeSbBc2M(this.paragraphInfoList, range, new Function1<ParagraphInfo, Unit>() { // from class: androidx.compose.ui.text.MultiParagraph$fillBoundingBoxes$1
            /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
            {
                super(1);
            }
            ...
            public final void invoke2(ParagraphInfo paragraphInfo) {
                ...
                long jTextRange = TextRangeKt.TextRange(paragraphInfo.toLocalIndex(paragraphInfo.getStartIndex() > TextRange.m5233getMinimpl(j) ? paragraphInfo.getStartIndex() : TextRange.m5233getMinimpl(j)), paragraphInfo.toLocalIndex(paragraphInfo.getEndIndex() < TextRange.m5232getMaximpl(j) ? paragraphInfo.getEndIndex() : TextRange.m5232getMaximpl(j)));
                paragraphInfo.getParagraph().mo5081fillBoundingBoxes8ffj60Q(jTextRange, fArr, intRef2.element);
                int iM5231getLengthimpl = intRef2.element + (TextRange.m5231getLengthimpl(jTextRange) * 4);
                for (int i = intRef2.element; i < iM5231getLengthimpl; i += 4) {
                    int i2 = i + 1;
                    fArr[i2] = fArr[i2] + floatRef2.element;
                    int i3 = i + 3;
                    fArr[i3] = fArr[i3] + floatRef2.element;
                }
                intRef2.element = iM5231getLengthimpl;
    ...
    public final long m5113getWordBoundaryjx7JFs(int offset) {
        int iFindParagraphByIndex;
        requireIndexInRangeInclusiveEnd(offset);
        if (offset == getAnnotatedString().length()) {
            iFindParagraphByIndex = CollectionsKt.getLastIndex(this.paragraphInfoList);
        ...
        ParagraphInfo paragraphInfo = this.paragraphInfoList.get(iFindParagraphByIndex);
        return paragraphInfo.m5124toGlobalGEjPoXI(paragraphInfo.getParagraph().mo5084getWordBoundaryjx7JFs(paragraphInfo.toLocalIndex(offset)));
    }
