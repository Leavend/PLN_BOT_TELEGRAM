...
public final class DeleteSurroundingTextInCodePointsCommand implements EditCommand {
    ...
    public void applyTo(EditingBuffer buffer) {
        ...
        for (int i3 = 0; i3 < i; i3++) {
            i2++;
            if (buffer.getSelectionStart() > i2 && EditCommandKt.isSurrogatePair(buffer.get$ui_text_release((buffer.getSelectionStart() - i2) - 1), buffer.get$ui_text_release(buffer.getSelectionStart() - i2))) {
                i2++;
            }
        ...
        for (int i6 = 0; i6 < i4; i6++) {
            i5++;
            if (buffer.getSelectionEnd() + i5 < buffer.getLength$ui_text_release() && EditCommandKt.isSurrogatePair(buffer.get$ui_text_release((buffer.getSelectionEnd() + i5) - 1), buffer.get$ui_text_release(buffer.getSelectionEnd() + i5))) {
                i5++;
            }
            if (buffer.getSelectionEnd() + i5 == buffer.getLength$ui_text_release()) {
                break;
            }
        }
        buffer.delete$ui_text_release(buffer.getSelectionEnd(), buffer.getSelectionEnd() + i5);
