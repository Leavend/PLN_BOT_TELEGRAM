...
public final class EditingBuffer {
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j) {
        ...
        this.selectionStart = TextRange.m5233getMinimpl(j);
        this.selectionEnd = TextRange.m5232getMaximpl(j);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int iM5233getMinimpl = TextRange.m5233getMinimpl(j);
        int iM5232getMaximpl = TextRange.m5232getMaximpl(j);
        if (iM5233getMinimpl < 0 || iM5233getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + iM5233getMinimpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM5232getMaximpl < 0 || iM5232getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + iM5232getMaximpl + ") offset is outside of text region " + annotatedString.length());
        }
        if (iM5233getMinimpl > iM5232getMaximpl) {
            throw new IllegalArgumentException("Do not set reversed range: " + iM5233getMinimpl + " > " + iM5232getMaximpl);
        }
    }
    ...
    public final TextRange m5394getCompositionMzsxiRA$ui_text_release() {
        if (hasComposition$ui_text_release()) {
            return TextRange.m5223boximpl(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd));
        }
        return null;
    }

    /* renamed from: getSelection-d9O1mEE$ui_text_release, reason: not valid java name */
    public final long m5395getSelectiond9O1mEE$ui_text_release() {
    ...
    public final void delete$ui_text_release(int start, int end) {
        ...
        long jM5396updateRangeAfterDeletepWDy79M = EditingBufferKt.m5396updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), jTextRange);
        setSelectionStart(TextRange.m5233getMinimpl(jM5396updateRangeAfterDeletepWDy79M));
        setSelectionEnd(TextRange.m5232getMaximpl(jM5396updateRangeAfterDeletepWDy79M));
        if (hasComposition$ui_text_release()) {
            long jM5396updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m5396updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), jTextRange);
            if (TextRange.m5229getCollapsedimpl(jM5396updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
            } else {
                this.compositionStart = TextRange.m5233getMinimpl(jM5396updateRangeAfterDeletepWDy79M2);
                this.compositionEnd = TextRange.m5232getMaximpl(jM5396updateRangeAfterDeletepWDy79M2);
            }
        }
    }
