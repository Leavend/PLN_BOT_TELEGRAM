...
public final class HitTestResult implements List<Modifier.Node>, KMappedMarker {
    ...
    public final boolean hasHit() {
        long jM4807findBestHitDistanceptXAw2c = m4807findBestHitDistanceptXAw2c();
        return DistanceAndInLayer.m4802getDistanceimpl(jM4807findBestHitDistanceptXAw2c) < 0.0f && DistanceAndInLayer.m4804isInLayerimpl(jM4807findBestHitDistanceptXAw2c);
    }
    ...
    public final boolean isHitInMinimumTouchTargetBetter(float distanceFromEdge, boolean isInLayer) {
        ...
        return DistanceAndInLayer.m4798compareToS_HNhKs(m4807findBestHitDistanceptXAw2c(), HitTestResultKt.DistanceAndInLayer(distanceFromEdge, isInLayer)) > 0;
    }

    /* renamed from: findBestHitDistance-ptXAw2c, reason: not valid java name */
    private final long m4807findBestHitDistanceptXAw2c() {
        long jDistanceAndInLayer = HitTestResultKt.DistanceAndInLayer(Float.POSITIVE_INFINITY, false);
        int i = this.hitDepth + 1;
        int lastIndex = CollectionsKt.getLastIndex(this);
        if (i <= lastIndex) {
            while (true) {
                long jM4799constructorimpl = DistanceAndInLayer.m4799constructorimpl(this.distanceFromEdgeAndInLayer[i]);
                if (DistanceAndInLayer.m4798compareToS_HNhKs(jM4799constructorimpl, jDistanceAndInLayer) < 0) {
                    jDistanceAndInLayer = jM4799constructorimpl;
                }
                if (DistanceAndInLayer.m4802getDistanceimpl(jDistanceAndInLayer) < 0.0f && DistanceAndInLayer.m4804isInLayerimpl(jDistanceAndInLayer)) {
    ...
    public final void speculativeHit(Modifier.Node node, float distanceFromEdge, boolean isInLayer, Function0<Unit> childHitTest) {
        ...
        hitInMinimumTouchTarget(node, distanceFromEdge, isInLayer, childHitTest);
        if (this.hitDepth + 1 < CollectionsKt.getLastIndex(hitTestResult) && DistanceAndInLayer.m4798compareToS_HNhKs(jM4807findBestHitDistanceptXAw2c, m4807findBestHitDistanceptXAw2c()) > 0) {
