...
public final class CommonRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        ...
        m1209drawRipples4WTKRHQ(contentDrawScope2, jM3424unboximpl);
    }
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        RippleAnimation rippleAnimation = new RippleAnimation(this.bounded ? Offset.m3162boximpl(interaction.getPressPosition()) : null, this.radius, this.bounded, null);
    ...
    private final void m1209drawRipples4WTKRHQ(DrawScope drawScope, long j) {
        Iterator<Map.Entry<PressInteraction.Press, RippleAnimation>> it = this.ripples.entrySet().iterator();
        while (it.hasNext()) {
            RippleAnimation value = it.next().getValue();
            float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
            if (!(pressedAlpha == 0.0f)) {
                value.m1211draw4WTKRHQ(drawScope, Color.m3413copywmQWz5c$default(j, pressedAlpha, 0.0f, 0.0f, 0.0f, 14, null));
            }
        }
    }
