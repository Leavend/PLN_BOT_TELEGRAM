...
public final class RippleHostView extends View {
    ...
    public final void m1215addRippleKOepWvA(PressInteraction.Press interaction, boolean bounded, long size, int radius, long color, float alpha, Function0<Unit> onInvalidateRipple) {
        ...
        m1216updateRipplePropertiesbiQXAtU(size, radius, color, alpha);
        if (bounded) {
            unprojectedRipple.setHotspot(Offset.m3173getXimpl(interaction.getPressPosition()), Offset.m3174getYimpl(interaction.getPressPosition()));
        } else {
    ...
    public final void m1216updateRipplePropertiesbiQXAtU(long size, int radius, long color, float alpha) {
        UnprojectedRipple unprojectedRipple = this.ripple;
        if (unprojectedRipple == null) {
            return;
        }
        unprojectedRipple.trySetRadius(radius);
        unprojectedRipple.m1223setColorDxMtmZc(color, alpha);
        Rect rect = new Rect(0, 0, MathKt.roundToInt(Size.m3242getWidthimpl(size)), MathKt.roundToInt(Size.m3239getHeightimpl(size)));
        setLeft(rect.left);
        setTop(rect.top);
        setRight(rect.right);
