...
public final class AndroidRippleIndicationInstance extends RippleIndicationInstance implements RememberObserver {
    ...
    private AndroidRippleIndicationInstance(boolean z, float f, State<Color> state, State<RippleAlpha> state2, ViewGroup viewGroup) {
        ...
        this.rippleSize = Size.INSTANCE.m3251getZeroNHjbRc();
        this.rippleRadius = -1;
    ...
    public void drawIndication(ContentDrawScope contentDrawScope) {
        int iRoundToInt;
        this.rippleSize = contentDrawScope.mo3963getSizeNHjbRc();
        if (Float.isNaN(this.radius)) {
            iRoundToInt = MathKt.roundToInt(RippleAnimationKt.m1212getRippleEndRadiuscSwnlzA(contentDrawScope, this.bounded, contentDrawScope.mo3963getSizeNHjbRc()));
        } else {
            iRoundToInt = contentDrawScope.mo310roundToPx0680j_4(this.radius);
        }
        this.rippleRadius = iRoundToInt;
        long jM3424unboximpl = this.color.getValue().m3424unboximpl();
        float pressedAlpha = this.rippleAlpha.getValue().getPressedAlpha();
        contentDrawScope.drawContent();
        ContentDrawScope contentDrawScope2 = contentDrawScope;
        m1217drawStateLayerH2RKhps(contentDrawScope2, this.radius, jM3424unboximpl);
        Canvas canvas = contentDrawScope2.getDrawContext().getCanvas();
        getInvalidateTick();
        RippleHostView rippleHostView = getRippleHostView();
        if (rippleHostView != null) {
            rippleHostView.m1216updateRipplePropertiesbiQXAtU(contentDrawScope.mo3963getSizeNHjbRc(), this.rippleRadius, jM3424unboximpl, pressedAlpha);
    ...
    public void addRipple(PressInteraction.Press interaction, CoroutineScope scope) {
        ...
        rippleHostView.m1215addRippleKOepWvA(interaction, this.bounded, this.rippleSize, this.rippleRadius, this.color.getValue().m3424unboximpl(), this.rippleAlpha.getValue().getPressedAlpha(), this.onInvalidateRipple);
