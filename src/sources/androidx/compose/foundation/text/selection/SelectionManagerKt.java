...
public final class SelectionManagerKt {
    ...
    public static final Rect getSelectedRegionRect(List<? extends Pair<? extends Selectable, Selection>> list, LayoutCoordinates layoutCoordinates) {
        ...
        while (i2 < size) {
            ...
            if (offset == offset2 || (layoutCoordinates2 = selectableComponent1.getLayoutCoordinates()) == null) {
                ...
                long jMo4692localPositionOfR5De75A = layoutCoordinates.mo4692localPositionOfR5De75A(layoutCoordinates2, jOffset);
                long jMo4692localPositionOfR5De75A2 = layoutCoordinates.mo4692localPositionOfR5De75A(layoutCoordinates2, jOffset2);
                left = Math.min(left, Offset.m3173getXimpl(jMo4692localPositionOfR5De75A));
                top = Math.min(top, Offset.m3174getYimpl(jMo4692localPositionOfR5De75A));
                right = Math.max(right, Offset.m3173getXimpl(jMo4692localPositionOfR5De75A2));
                bottom = Math.max(bottom, Offset.m3174getYimpl(jMo4692localPositionOfR5De75A2));
            }
            i2++;
            size = i;
        }
    ...
    public static final long m1044calculateSelectionMagnifierCenterAndroidO0kMr_c(SelectionManager selectionManager, long j) {
        ...
        if (i == 1) {
            return m1046getMagnifierCenterJVtK1S4(selectionManager, j, selection.getStart());
        }
        if (i == 2) {
            return m1046getMagnifierCenterJVtK1S4(selectionManager, j, selection.getEnd());
        }
        if (i == 3) {
    ...
    private static final long m1046getMagnifierCenterJVtK1S4(SelectionManager selectionManager, long j, Selection.AnchorInfo anchorInfo) {
        ...
        if (anchorSelectable$foundation_release != null && (containerLayoutCoordinates = selectionManager.getContainerLayoutCoordinates()) != null && (layoutCoordinates = anchorSelectable$foundation_release.getLayoutCoordinates()) != null && (offset = anchorInfo.getOffset()) <= anchorSelectable$foundation_release.getLastVisibleOffset()) {
            Offset offsetM1033getCurrentDragPosition_m7T9E = selectionManager.m1033getCurrentDragPosition_m7T9E();
            Intrinsics.checkNotNull(offsetM1033getCurrentDragPosition_m7T9E);
            float fM3173getXimpl = Offset.m3173getXimpl(layoutCoordinates.mo4692localPositionOfR5De75A(containerLayoutCoordinates, offsetM1033getCurrentDragPosition_m7T9E.getPackedValue()));
            long jMo994getRangeOfLineContainingjx7JFs = anchorSelectable$foundation_release.mo994getRangeOfLineContainingjx7JFs(offset);
            if (TextRange.m5229getCollapsedimpl(jMo994getRangeOfLineContainingjx7JFs)) {
                fCoerceIn = anchorSelectable$foundation_release.getLineLeft(offset);
            } else {
                float lineLeft = anchorSelectable$foundation_release.getLineLeft(TextRange.m5235getStartimpl(jMo994getRangeOfLineContainingjx7JFs));
                float lineRight = anchorSelectable$foundation_release.getLineRight(TextRange.m5230getEndimpl(jMo994getRangeOfLineContainingjx7JFs) - 1);
                fCoerceIn = RangesKt.coerceIn(fM3173getXimpl, Math.min(lineLeft, lineRight), Math.max(lineLeft, lineRight));
            }
            if (fCoerceIn == -1.0f) {
                return Offset.INSTANCE.m3188getUnspecifiedF1C5BW0();
            }
            if (Math.abs(fM3173getXimpl - fCoerceIn) > IntSize.m5905getWidthimpl(j) / 2) {
                return Offset.INSTANCE.m3188getUnspecifiedF1C5BW0();
            }
            ...
            return containerLayoutCoordinates.mo4692localPositionOfR5De75A(layoutCoordinates, OffsetKt.Offset(fCoerceIn, centerYForOffset));
        }
        return Offset.INSTANCE.m3188getUnspecifiedF1C5BW0();
    }
    ...
    public static final Rect visibleBounds(LayoutCoordinates layoutCoordinates) {
        ...
        return RectKt.m3211Rect0a9Yr6o(layoutCoordinates.mo4696windowToLocalMKHz9U(rectBoundsInWindow.m3208getTopLeftF1C5BW0()), layoutCoordinates.mo4696windowToLocalMKHz9U(rectBoundsInWindow.m3202getBottomRightF1C5BW0()));
    }

    /* renamed from: containsInclusive-Uv8p0NA, reason: not valid java name */
    public static final boolean m1045containsInclusiveUv8p0NA(Rect rect, long j) {
        float left = rect.getLeft();
        float right = rect.getRight();
        float fM3173getXimpl = Offset.m3173getXimpl(j);
        if (left <= fM3173getXimpl && fM3173getXimpl <= right) {
            float top = rect.getTop();
            float bottom = rect.getBottom();
            float fM3174getYimpl = Offset.m3174getYimpl(j);
            if (top <= fM3174getYimpl && fM3174getYimpl <= bottom) {
                return true;
            }
        }
        return false;
