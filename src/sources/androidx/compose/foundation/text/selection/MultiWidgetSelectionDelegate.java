...
public final class MultiWidgetSelectionDelegate implements Selectable {
    ...
    private final synchronized int getLastVisibleOffset(TextLayoutResult textLayoutResult) {
        ...
        if (this._previousTextLayoutResult != textLayoutResult) {
            if (!textLayoutResult.getDidOverflowHeight() || textLayoutResult.getMultiParagraph().getDidExceedMaxLines()) {
                ...
                int iCoerceAtMost = RangesKt.coerceAtMost(textLayoutResult.getLineForVerticalPosition(IntSize.m5904getHeightimpl(textLayoutResult.getSize())), textLayoutResult.getLineCount() - 1);
                while (iCoerceAtMost >= 0 && textLayoutResult.getLineTop(iCoerceAtMost) >= IntSize.m5904getHeightimpl(textLayoutResult.getSize())) {
                    iCoerceAtMost--;
                }
    ...
    public void appendSelectableInfoToBuilder(SelectionLayoutBuilder builder) {
        ...
        long jM3177minusMKHz9U;
        LayoutCoordinates layoutCoordinates = getLayoutCoordinates();
        if (layoutCoordinates == null || (textLayoutResultInvoke = this.layoutResultCallback.invoke()) == null) {
            return;
        }
        long jMo4692localPositionOfR5De75A = builder.getContainerCoordinates().mo4692localPositionOfR5De75A(layoutCoordinates, Offset.INSTANCE.m3189getZeroF1C5BW0());
        long jM3177minusMKHz9U2 = Offset.m3177minusMKHz9U(builder.getCurrentPosition(), jMo4692localPositionOfR5De75A);
        if (OffsetKt.m3194isUnspecifiedk4lQ0M(builder.getPreviousHandlePosition())) {
            jM3177minusMKHz9U = Offset.INSTANCE.m3188getUnspecifiedF1C5BW0();
        } else {
            jM3177minusMKHz9U = Offset.m3177minusMKHz9U(builder.getPreviousHandlePosition(), jMo4692localPositionOfR5De75A);
        }
        MultiWidgetSelectionDelegateKt.m995appendSelectableInfoParwq6A(builder, textLayoutResultInvoke, jM3177minusMKHz9U2, jM3177minusMKHz9U, getSelectableId());
    }
    ...
    public long mo993getHandlePositiondBAh8RU(Selection selection, boolean isStartHandle) {
        TextLayoutResult textLayoutResultInvoke;
    ...
    public long mo994getRangeOfLineContainingjx7JFs(int offset) {
        TextLayoutResult textLayoutResultInvoke = this.layoutResultCallback.invoke();
        if (textLayoutResultInvoke == null) {
