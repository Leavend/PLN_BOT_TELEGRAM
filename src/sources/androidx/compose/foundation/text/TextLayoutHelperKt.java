...
public final class TextLayoutHelperKt {
    ...
    public static final boolean m939canReuse7_7YC6M(TextLayoutResult textLayoutResult, AnnotatedString annotatedString, TextStyle textStyle, List<AnnotatedString.Range<Placeholder>> list, int i, boolean z, int i2, Density density, LayoutDirection layoutDirection, FontFamily.Resolver resolver, long j) {
        TextLayoutInput layoutInput = textLayoutResult.getLayoutInput();
        if (textLayoutResult.getMultiParagraph().getIntrinsics().getHasStaleResolvedFonts() || !Intrinsics.areEqual(layoutInput.getText(), annotatedString) || !layoutInput.getStyle().hasSameLayoutAffectingAttributes(textStyle) || !Intrinsics.areEqual(layoutInput.getPlaceholders(), list) || layoutInput.getMaxLines() != i || layoutInput.getSoftWrap() != z || !TextOverflow.m5669equalsimpl0(layoutInput.getOverflow(), i2) || !Intrinsics.areEqual(layoutInput.getDensity(), density) || layoutInput.getLayoutDirection() != layoutDirection || !Intrinsics.areEqual(layoutInput.getFontFamilyResolver(), resolver) || Constraints.m5693getMinWidthimpl(j) != Constraints.m5693getMinWidthimpl(layoutInput.getConstraints())) {
            return false;
        }
        if (z || TextOverflow.m5669equalsimpl0(i2, TextOverflow.INSTANCE.m5677getEllipsisgIe3tQ8())) {
            return Constraints.m5691getMaxWidthimpl(j) == Constraints.m5691getMaxWidthimpl(layoutInput.getConstraints()) && Constraints.m5690getMaxHeightimpl(j) == Constraints.m5690getMaxHeightimpl(layoutInput.getConstraints());
        }
        return true;
    }
}
