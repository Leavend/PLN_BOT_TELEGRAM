...
public final class LazyGridAnimateScrollScope implements LazyLayoutAnimateScrollScope {
    ...
    public int getVisibleItemScrollOffset(int index) {
        ...
        if (this.state.getIsVertical()) {
            iM5863getXimpl = IntOffset.m5864getYimpl(lazyGridItemInfo2.getOffset());
        } else {
            iM5863getXimpl = IntOffset.m5863getXimpl(lazyGridItemInfo2.getOffset());
        }
        return iM5863getXimpl;
    }
        ...
        while (i < visibleItemsInfo.size()) {
            ...
            if (iIntValue == -1) {
                ...
                while (i < visibleItemsInfo.size() && function1.invoke(Integer.valueOf(i)).intValue() == iIntValue) {
                    if (isVertical) {
                        iM5905getWidthimpl = IntSize.m5904getHeightimpl(visibleItemsInfo.get(i).getSize());
                    } else {
                        iM5905getWidthimpl = IntSize.m5905getWidthimpl(visibleItemsInfo.get(i).getSize());
                    }
                    iMax = Math.max(iMax, iM5905getWidthimpl);
                    i++;
                }
                i2 += iMax;
                i3++;
