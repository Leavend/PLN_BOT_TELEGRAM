...
public final class AbsoluteCutCornerShape extends CornerBasedShape {
    ...
    public Outline mo819createOutlineLjSzlW0(long size, float topStart, float topEnd, float bottomEnd, float bottomStart, LayoutDirection layoutDirection) {
        if (((topStart + topEnd) + bottomStart) + bottomEnd == 0.0f) {
            return new Outline.Rectangle(SizeKt.m3263toRectuvyYCjk(size));
        }
        Path Path = AndroidPath_androidKt.Path();
        Path.moveTo(0.0f, topStart);
        Path.lineTo(topStart, 0.0f);
        Path.lineTo(Size.m3242getWidthimpl(size) - topEnd, 0.0f);
        Path.lineTo(Size.m3242getWidthimpl(size), topEnd);
        Path.lineTo(Size.m3242getWidthimpl(size), Size.m3239getHeightimpl(size) - bottomEnd);
        Path.lineTo(Size.m3242getWidthimpl(size) - bottomEnd, Size.m3239getHeightimpl(size));
        Path.lineTo(bottomStart, Size.m3239getHeightimpl(size));
        Path.lineTo(0.0f, Size.m3239getHeightimpl(size) - bottomStart);
        Path.close();
