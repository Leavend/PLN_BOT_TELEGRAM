...
final class SplineBasedFloatDecayAnimationSpec implements FloatDecayAnimationSpec {
    ...
    public float getValueFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        ...
        return initialValue + (flingDistance(initialVelocity) * AndroidFlingSpline.FlingResult.m469getDistanceCoefficientimpl(AndroidFlingSpline.INSTANCE.m464flingPositionLfoxSSI(durationNanos > 0 ? playTimeNanos / durationNanos : 1.0f)));
    }
    ...
    public float getVelocityFromNanos(long playTimeNanos, float initialValue, float initialVelocity) {
        long durationNanos = getDurationNanos(0.0f, initialVelocity);
        return ((AndroidFlingSpline.FlingResult.m470getVelocityCoefficientimpl(AndroidFlingSpline.INSTANCE.m464flingPositionLfoxSSI(durationNanos > 0 ? playTimeNanos / durationNanos : 1.0f)) * flingDistance(initialVelocity)) / durationNanos) * 1.0E9f;
    }
}
