...
final class IntrinsicHeightNode extends IntrinsicSizeModifier {
    ...
    public long mo519calculateContentConstraintsl58MMJ0(MeasureScope measureScope, Measurable measurable, long j) {
        int iMaxIntrinsicHeight;
        if (this.height == IntrinsicSize.Min) {
            iMaxIntrinsicHeight = measurable.minIntrinsicHeight(Constraints.m5691getMaxWidthimpl(j));
        } else {
            iMaxIntrinsicHeight = measurable.maxIntrinsicHeight(Constraints.m5691getMaxWidthimpl(j));
        }
        if (iMaxIntrinsicHeight < 0) {
            iMaxIntrinsicHeight = 0;
        }
        return Constraints.INSTANCE.m5700fixedHeightOenEA2s(iMaxIntrinsicHeight);
    }
