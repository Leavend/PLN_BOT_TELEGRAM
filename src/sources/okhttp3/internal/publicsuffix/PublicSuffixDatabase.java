...
public final class PublicSuffixDatabase {
    ...
    private final List<String> findMatchingRule(List<String> domainLabels) throws InterruptedException {
        ...
        for (int i = 0; i < size; i++) {
            byte[] bytes = domainLabels.get(i).getBytes(Charsets.UTF_8);
            ...
            while (i3 < length) {
                ...
                while (true) {
                    if (z2) {
                        ...
                        z = z2;
                        iAnd = _UtilCommonKt.and(bArr2[i8][i9], 255);
                    }
                    iAnd2 = iAnd - _UtilCommonKt.and(bArr[i5 + i10], 255);
                    if (iAnd2 != 0) {
                        break;
                    }
                    i10++;
